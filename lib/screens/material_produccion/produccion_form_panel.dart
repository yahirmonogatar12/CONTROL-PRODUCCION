...
import 'package:control_produccion_flutter/core/widgets/printer_settings_dialog.dart';
import 'package:control_produccion_flutter/core/services/api_service.dart';
import 'package:control_produccion_flutter/core/services/printer_service.dart';
import 'package:control_produccion_flutter/core/services/auth_service.dart';
import 'produccion_grid_panel.dart';

class ProduccionFormPanel extends StatefulWidget {
  final LanguageProvider languageProvider;
  final VoidCallback? onDataSaved; // Callback cuando se guardan datos
  final GlobalKey<ProduccionGridPanelState>? gridKey; // Referencia al grid para reimprimir
  
  const ProduccionFormPanel({
    super.key, 
  ...
  @override
  State<ProduccionFormPanel> createState() => ProduccionFormPanelState();
}

class ProduccionFormPanelState extends State<ProduccionFormPanel> {
  ...
  Future<void> _confirmWarehouseEntry() async {
    ...
    // En modo Entrada Directa: crear registro directamente en control_material_almacen_prod
    ...
    if (result['success'] == true) {
      // Verificar si realmente se insertó el registro en almacen_prod
      final insertedAlmacen = result['inserted_almacen'];
      if (insertedAlmacen == false) {
        // El material ya existía en control_material_almacen_prod, no se insertó nuevo
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
