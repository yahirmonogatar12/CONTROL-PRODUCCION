...
class _MobileInventoryScreenState extends State<MobileInventoryScreen> {
  ...
  Future<void> _searchInventory(String query) async {
    ...
    try {
      // Extraer número de parte de código escaneado (ej: EBC36198401-202601120002 → EBC36198401)
      final partNumber = _extractPartNumber(query);

      // Buscar por número de parte extraído y código completo
      final results = await ApiService.getInventoryLots(
        numeroParte: partNumber,
        codigoMaterialRecibido: query,
      );

      // Si no hay resultados por lotes, buscar en resumen
      if (results.isEmpty) {
        final summary = await ApiService.getInventorySummary(numeroParte: partNumber);
        setState(() {
          _inventoryResults = summary;
          _isLoading = false;
        });
