...
class ServerProfile {
  ...
  bool get _useStandardPort => (useHttps && port == 443) || (!useHttps && port == 80);

  /// Get the full API base URL for this server
  String get baseUrl => _useStandardPort 
      ? '$protocol://$ip/api' 
      : '$protocol://$ip:$port/api';
...
class ServerConfig {
  ...
  static Future<void> init() async {
    ...
    if (serversJson != null) {
      try {
        ...
        _servers = serversList
            .map((s) => ServerProfile.fromJson(s))
            .toList();
      } catch (e) {
    ...
    if (_activeServerId != null && !_servers.any((s) => s.id == _activeServerId)) {
      _activeServerId = _servers.first.id;
      await _saveToPrefs();
    }

    _isInitialized = true;
    print('ServerConfig initialized with ${_servers.length} servers. Active: ${activeServer?.displayString}');
  }
  ...
  static Future<void> addServer(ServerProfile server) async {
    ...
    final newServer = server.copyWith(
      id: server.id.isEmpty ? DateTime.now().millisecondsSinceEpoch.toString() : server.id,
    );
  ...
  static Future<bool> testConnection({String? ip, int? port, bool? useHttps}) async {
    ...
    final useStandardPort = (testHttps && testPort == 443) || (!testHttps && testPort == 80);
