...
async function run() {
  ...
  try {
    console.log('\n🔧 Actualizando tabla iqc_inspection_detail_prod...\n');
    
    // Verificar si la tabla existe
    const [tables] = await pool.query("SHOW TABLES LIKE 'iqc_inspection_detail_prod'");
    
    if (tables.length === 0) {
      console.log('⚠️ Tabla iqc_inspection_detail_prod no existe. Creándola...');
      await pool.query(`
        CREATE TABLE iqc_inspection_detail_prod (
      ...
      const [columns] = await pool.query("DESCRIBE iqc_inspection_detail_prod");
      ...
      if (resultCol) {
        ...
        if (resultCol.Type.includes('enum')) {
          ...
          await pool.query("ALTER TABLE iqc_inspection_detail_prod MODIFY COLUMN result VARCHAR(20) NOT NULL");
      ...
      if (testNameCol) {
        ...
        if (testNameCol.Null === 'NO') {
          ...
          await pool.query("ALTER TABLE iqc_inspection_detail_prod MODIFY COLUMN test_name VARCHAR(100) NULL");
          console.log('   ✅ Columna test_name actualizada');
        }
      } else {
        console.log('   Agregando columna test_name...');
        await pool.query("ALTER TABLE iqc_inspection_detail_prod ADD COLUMN test_name VARCHAR(100) NULL AFTER characteristic");
      ...
      if (characteristicCol) {
        ...
        if (characteristicCol.Type.includes('enum')) {
          ...
          await pool.query("ALTER TABLE iqc_inspection_detail_prod MODIFY COLUMN characteristic VARCHAR(50) NOT NULL");
          console.log('   ✅ Columna characteristic actualizada');
        }
      }
    }

    console.log('\n✅ Tabla iqc_inspection_detail_prod lista!\n');

    // Mostrar estructura final
    console.log('=== Estructura final ===');
    const [finalCols] = await pool.query("DESCRIBE iqc_inspection_detail_prod");
