...
async function addUnitColumn() {
  ...
  try {
    ...
    // 2. Check and add to control_material_almacen_prod table
    const [whColumns] = await pool.query(`
      SELECT COLUMN_NAME 
      FROM INFORMATION_SCHEMA.COLUMNS 
      WHERE TABLE_SCHEMA = DATABASE() 
        AND TABLE_NAME = 'control_material_almacen_prod' 
        AND COLUMN_NAME = 'unidad_medida'
    `);

    if (whColumns.length > 0) {
      console.log('✓ Column unidad_medida already exists in control_material_almacen_prod');
    } else {
      await pool.query(`
      ...
      `);
      console.log('✓ Column unidad_medida added to control_material_almacen_prod');
    }

    // 3. Check and add to inventario_lotes_prod table
    const [invColumns] = await pool.query(`
      SELECT COLUMN_NAME 
      FROM INFORMATION_SCHEMA.COLUMNS 
      WHERE TABLE_SCHEMA = DATABASE() 
        AND TABLE_NAME = 'inventario_lotes_prod' 
        AND COLUMN_NAME = 'unidad_medida'
    `);

    if (invColumns.length > 0) {
      console.log('✓ Column unidad_medida already exists in inventario_lotes_prod');
    } else {
      await pool.query(`
        ALTER TABLE inventario_lotes_prod 
        ADD COLUMN unidad_medida VARCHAR(10) DEFAULT 'EA' 
        AFTER stock_actual
      `);
      console.log('✓ Column unidad_medida added to inventario_lotes_prod');
    }
    ...
    const [whUpdated] = await pool.query(`
      UPDATE control_material_almacen_prod SET unidad_medida = 'EA' WHERE unidad_medida IS NULL OR unidad_medida = ''
    `);
    console.log(`✓ Updated ${whUpdated.affectedRows} warehousing records with default 'EA'`);

    const [invUpdated] = await pool.query(`
      UPDATE inventario_lotes_prod SET unidad_medida = 'EA' WHERE unidad_medida IS NULL OR unidad_medida = ''
    `);
