-- SQL idempotente para crear tablas `_prod` y sus triggers asociados
-- Ruta: backend/sql/apply_prod_triggers.sql
-- Recomendación: ejecutar en STAGING primero. HACER BACKUP antes de aplicar.

-- 1) Crear tablas fork (copian estructura + índices)
CREATE TABLE IF NOT EXISTS control_material_salida_prod LIKE control_material_salida;
CREATE TABLE IF NOT EXISTS inventario_lotes_prod LIKE inventario_lotes;
CREATE TABLE IF NOT EXISTS material_return_prod LIKE material_return;

-- 2) Triggers: control_material_almacen_prod (ENTRY) - INSERT
DROP TRIGGER IF EXISTS trg_almacen_ai_prod;
DELIMITER $$
CREATE TRIGGER trg_almacen_ai_prod
AFTER INSERT ON control_material_almacen_prod
FOR EACH ROW
BEGIN
  -- `cancelado` no existe en la tabla _prod: validar solo IQC
  IF NEW.iqc_status IN ('Released', 'NotRequired') THEN
    INSERT INTO inventario_lotes_prod (
...
-- 3) Triggers: control_material_almacen_prod (ENTRY) - UPDATE
DROP TRIGGER IF EXISTS trg_almacen_au_prod;
DELIMITER $$
CREATE TRIGGER trg_almacen_au_prod
AFTER UPDATE ON control_material_almacen_prod
FOR EACH ROW
BEGIN
...
-- 4) Trigger: control_material_salida_prod (EXIT) - INSERT
DROP TRIGGER IF EXISTS trg_salida_ai_prod;
DELIMITER $$
CREATE TRIGGER trg_salida_ai_prod
AFTER INSERT ON control_material_salida_prod
FOR EACH ROW
INSERT INTO inventario_lotes_prod (
...
-- 5) Trigger: material_return_prod (RETURN) - INSERT (adaptado a columnas _prod)
DROP TRIGGER IF EXISTS trg_return_ai_prod;
DELIMITER $$
CREATE TRIGGER trg_return_ai_prod
AFTER INSERT ON material_return_prod
FOR EACH ROW
BEGIN
  -- ajustar total_salida en inventario_lotes_prod cuando hay devolución
  UPDATE inventario_lotes_prod
...
DROP TRIGGER IF EXISTS trg_mounter_ai_prod;
DELIMITER $$
CREATE TRIGGER trg_mounter_ai_prod
...
-- 7) Backfill opcional (DESCOMENTAR si quieres copiar datos existentes a las tablas _prod)
-- INSERT INTO inventario_lotes_prod SELECT * FROM inventario_lotes;
-- INSERT INTO control_material_salida_prod SELECT * FROM control_material_salida;

-- 8) Verificaciones rápidas
SELECT 'tables' AS what, TABLE_NAME FROM INFORMATION_SCHEMA.TABLES
 WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME IN (
   'control_material_salida_prod','inventario_lotes_prod','material_return_prod'
 );

SELECT TRIGGER_NAME, EVENT_OBJECT_TABLE, ACTION_TIMING, EVENT_MANIPULATION
 FROM INFORMATION_SCHEMA.TRIGGERS
 WHERE TRIGGER_SCHEMA = DATABASE() AND TRIGGER_NAME LIKE 'trg\\_%\\_prod';
