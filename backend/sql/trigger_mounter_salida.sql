-- ============================================================================
-- Trigger: trg_mounter_ai_prod
-- Tabla origen: historial_cambio_material_smt (escaneo en mounters SMD)
-- Accion: Genera salida automatica en control_material_salida_prod
...
--
-- Reglas:
--   1. El Barcode debe existir en control_material_almacen_prod
...
--   5. Si ya tiene salida en control_material_salida_prod, se ignora (duplicado)
...
--   - El trigger trg_return_ai_prod resta del total_salida en inventario_lotes_prod
--
...
BEGIN
  ...
  IF v_almacen_id IS NOT NULL THEN
    ...
    -- -----------------------------------------------------------
    -- NOTA: No se necesita actualizar inventario_lotes_prod aqui
    -- porque el trigger existente trg_salida_ai_prod se dispara
    -- automaticamente al insertar en control_material_salida_prod
