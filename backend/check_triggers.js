...
async function run() {
  ...
  try {
    ...
    await pool.query('UPDATE inventario_lotes_prod SET total_entrada = 0');
    
    // Luego, recalcular solo con materiales Released/NotRequired
    const recalcularQuery = `
      UPDATE inventario_lotes_prod il
      SET total_entrada = (
    ...
    console.log('=== Triggers actuales en control_material_almacen_prod ===');
    const [triggers] = await pool.query("SHOW TRIGGERS WHERE `Table` = 'control_material_almacen_prod'");
