...
async function run() {
  ...
  try {
    ...
    const [columns] = await pool.query("DESCRIBE iqc_inspection_lot_prod");
    ...
    for (const field of resultFields) {
      try {
        await pool.query(`ALTER TABLE iqc_inspection_lot_prod MODIFY COLUMN ${field} VARCHAR(20) DEFAULT 'Pending'`);
    ...
    console.log('\n=== Estructura final ===\n');
    const [finalColumns] = await pool.query("DESCRIBE iqc_inspection_lot_prod");
