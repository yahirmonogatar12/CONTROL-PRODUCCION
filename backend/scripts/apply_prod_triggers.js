#!/usr/bin/env node
// Ejecuta backend/sql/apply_prod_triggers.sql de forma segura (ejecuta bloques CREATE TRIGGER por separado)
// Uso: node backend/scripts/apply_prod_triggers.js
...
const SQL_PATH = path.resolve(__dirname, '..', 'sql', 'apply_prod_triggers.sql');
...
(async () => {
  console.log('⚙️  apply_prod_triggers: comenzando (usar en staging, haga backup antes)');
  ...
  try {
    ...
    // Trigger definitions (adaptadas al esquema _prod)
    const triggerDefs = [
      {
        name: 'trg_almacen_ai_prod',
        sql: `DROP TRIGGER IF EXISTS trg_almacen_ai_prod; CREATE TRIGGER trg_almacen_ai_prod
...
END;`
      ...
      {
        name: 'trg_almacen_au_prod',
        sql: `DROP TRIGGER IF EXISTS trg_almacen_au_prod; CREATE TRIGGER trg_almacen_au_prod
...
END;`
      ...
      {
        name: 'trg_salida_ai_prod',
        sql: `DROP TRIGGER IF EXISTS trg_salida_ai_prod; CREATE TRIGGER trg_salida_ai_prod
...
ON DUPLICATE KEY UPDATE
  ...
  ultima_salida = GREATEST(ultima_salida, NEW.fecha_salida);`
      ...
      {
        name: 'trg_return_ai_prod',
        sql: `DROP TRIGGER IF EXISTS trg_return_ai_prod; CREATE TRIGGER trg_return_ai_prod
...
END;`
    ...
    const [rows] = await conn.query(
      "SELECT TRIGGER_NAME, EVENT_OBJECT_TABLE, ACTION_TIMING, EVENT_MANIPULATION FROM INFORMATION_SCHEMA.TRIGGERS WHERE TRIGGER_SCHEMA = DATABASE() AND TRIGGER_NAME LIKE '%_prod'"
    );

    console.log('\n✅ triggers `_prod` instalados/encontrados:');
    rows.forEach(r => console.log(` - ${r.TRIGGER_NAME}  on ${r.EVENT_OBJECT_TABLE}  (${r.ACTION_TIMING} ${r.EVENT_MANIPULATION})`));

    console.log('\n🔎 Revisa `backend/sql/test_prod_triggers.sql` y ejecuta las pruebas en staging.');
  } catch (err) {
