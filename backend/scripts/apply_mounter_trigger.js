/**
 * Script para aplicar el trigger trg_mounter_ai_prod
 * 
...
async function applyMounterTrigger() {
  ...
  try {
    ...
    await connection.query('DROP TRIGGER IF EXISTS trg_mounter_ai_prod');
    ...
    console.log('Creando trigger trg_mounter_ai_prod...');
    ...
    const [triggers] = await connection.query(`
      ...
      WHERE TRIGGER_SCHEMA = DATABASE()
        AND TRIGGER_NAME = 'trg_mounter_ai_prod'
    `);
    ...
    console.log('  2. Busca Barcode en control_material_almacen_prod (tiene_salida=0, cancelado=0)');
    console.log('  3. INSERT en control_material_salida_prod (depto=SMD, proceso=Mounter)');
    console.log('  4. UPDATE tiene_salida=1 en control_material_almacen_prod');
    console.log('  5. trg_salida_ai_prod actualiza inventario_lotes_prod automaticamente');
    console.log('\nMapeo de lineas:');
