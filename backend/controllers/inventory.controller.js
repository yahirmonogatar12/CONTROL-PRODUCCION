...
exports.getSummary = async (req, res, next) => {
  try {
    ...
    if (fecha_inicio && fecha_fin) {
      ...
      let query = `
        ...
        LEFT JOIN (
          ...
          FROM control_material_salida_prod
    ...
    // Inventario actual desde inventario_lotes_prod
    let query = `
      SELECT
        il.numero_parte,
        MAX(COALESCE(m.especificacion_material, cma.especificacion)) as especificacion,
      ...
      FROM inventario_lotes_prod il
      LEFT JOIN control_material_almacen_prod cma ON il.codigo_material_recibido = cma.codigo_material_recibido
...
exports.getLots = async (req, res, next) => {
  try {
    ...
    // detectar si la columna en_cuarentena existe en control_material_almacen_prod (evita errores en instalaciones parciales)
    const [[colCheck]] = await pool.query(
      `SELECT COUNT(*) AS cnt FROM INFORMATION_SCHEMA.COLUMNS
       WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'control_material_almacen_prod' AND COLUMN_NAME = 'en_cuarentena'`
    );
    ...
    let query = `
      SELECT
        ...
        (
          ...
          FROM control_material_salida_prod cms
