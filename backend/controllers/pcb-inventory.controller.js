...
exports.scan = async (req, res, next) => {
  ...
  try {
      ...
      const [sourceRows] = await connection.query(
        `SELECT entrada.*,
          entrada.qty - COALESCE((
            ...
            FROM pcb_inventory_scan_prod salida
         ...
         FROM pcb_inventory_scan_prod entrada
    ...
    const [existing] = await connection.query(
      `SELECT id, area FROM pcb_inventory_scan_prod
...
exports.deleteScan = async (req, res, next) => {
  try {
    ...
    const [result] = await pool.query(
      `DELETE FROM pcb_inventory_scan_prod WHERE id = ?`,
      [id]
    );
