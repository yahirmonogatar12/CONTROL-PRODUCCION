 ...
 *
 * Tablas clave:
 * - inventory_audit_prod: sesion y totales de la auditoria
 * - inventory_audit_location_prod: ubicaciones incluidas y progreso
 * - inventory_audit_item_prod: items verificados (Found/Missing/ProcessedOut)
 *
 * Alcance: solo material activo de control_material_almacen_prod
 * (cantidad_actual > 0, sin cancelado/desecho/salida y con ubicacion_salida valida).
 */
...
const getActiveAudit = async (req, res, next) => {
  try {
    const [rows] = await pool.query(`
      SELECT * FROM inventory_audit_prod 
...
// 3) Toma ubicaciones con inventario activo y crea inventory_audit_location_prod.
// 4) Crea inventory_audit_part_prod agrupando por ubicacion + numero_parte.
// 5) Inserta inventory_audit_prod con totales globales.
const startAudit = async (req, res, next) => {
  try {
    ...
    const [existing] = await pool.query(`
      SELECT id FROM inventory_audit_prod 
      WHERE status IN ('Pending', 'InProgress')
    `);
    ...
    // Obtener todas las ubicaciones con material activo
    // Regla: solo material disponible, no cancelado, no desecho y sin salida
    const [locations] = await pool.query(`
      SELECT DISTINCT ubicacion_salida as location,
      ...
      WHERE cantidad_actual > 0 
        ...
        AND ubicacion_salida IS NOT NULL 
        AND ubicacion_salida != ''
      GROUP BY ubicacion_salida
      ORDER BY ubicacion_salida
    `);

    // Crear auditoria con resumen global (ubicaciones e items esperados)
    const [result] = await pool.query(`
      INSERT INTO inventory_audit_prod (
    ...
    for (const loc of locations) {
      await pool.query(`
        INSERT INTO inventory_audit_location_prod (
    ...
    const [partSummary] = await pool.query(`
      SELECT 
        ubicacion_salida as location,
        numero_parte,
        COUNT(*) as expected_items,
        SUM(cantidad_actual) as expected_qty
      FROM control_material_almacen_prod
      WHERE cantidad_actual > 0 
        AND (cancelado = 0 OR cancelado IS NULL)
        AND (estado_desecho = 0 OR estado_desecho IS NULL)
      ...
      ORDER BY ubicacion_salida, numero_parte
    `);

    // Insertar registros de partes
    for (const part of partSummary) {
      await pool.query(`
        INSERT INTO inventory_audit_part_prod (
...
const endAudit = async (req, res, next) => {
  ...
  try {
    ...
    const [audit] = await connection.query(`
      SELECT * FROM inventory_audit_prod WHERE id = ? AND status = 'InProgress'
    `, [auditId]);
    ...
    // Estas cifras quedan guardadas en inventory_audit_prod para reportes
    const [stats] = await connection.query(`
      ...
      FROM inventory_audit_location_prod
      WHERE audit_id = ?
    `, [auditId]);
    ...
    const [itemStats] = await connection.query(`
      ...
      FROM inventory_audit_item_prod
      WHERE audit_id = ?
    `, [auditId]);
    ...
    if (confirmar_discrepancias) {
      ...
      const [locations] = await connection.query(`
        SELECT location FROM inventory_audit_location_prod WHERE audit_id = ?
      `, [auditId]);
      ...
      for (const loc of locations) {
        ...
        const [unscannedItems] = await connection.query(`
          SELECT cma.id as warehousing_id, cma.codigo_material_recibido, cma.ubicacion_salida
          FROM control_material_almacen_prod cma
          LEFT JOIN inventory_audit_item_prod iai ON iai.warehousing_id = cma.id AND iai.audit_id = ?
          WHERE cma.ubicacion_salida = ?
            AND cma.cantidad_actual > 0
            AND (cma.cancelado = 0 OR cma.cancelado IS NULL)
            AND (cma.estado_desecho = 0 OR cma.estado_desecho IS NULL)
            AND (cma.tiene_salida = 0 OR cma.tiene_salida IS NULL)
        ...
        for (const item of unscannedItems) {
          ...
          const [existing] = await connection.query(`
            SELECT id FROM inventory_audit_item_prod WHERE audit_id = ? AND warehousing_id = ?
          `, [auditId, item.warehousing_id]);

          if (existing.length === 0) {
            // Crear registro Missing para marcar que el material no aparecio en conteo
            await connection.query(`
              INSERT INTO inventory_audit_item_prod (
                audit_id, warehousing_id, warehousing_code, location, status, scanned_at, scanned_by
              ) VALUES (?, ?, ?, ?, 'Missing', NOW(), ?)
            `, [auditId, item.warehousing_id, item.codigo_material_recibido, item.ubicacion_salida, usuario_fin || 'Sistema-NoEncontrado']);
          } else {
            // Actualizar a Missing si esta Pending (no escaneado)
            await connection.query(`
              UPDATE inventory_audit_item_prod SET status = 'Missing', scanned_at = NOW(), scanned_by = ?
      ...
      // Esto crea registros en control_material_salida_prod y marca el material como desecho/salida
      const [missingItems] = await connection.query(`
        SELECT iai.*, cma.numero_parte, cma.numero_lote_material, cma.cantidad_actual, cma.especificacion
        FROM inventory_audit_item_prod iai
        JOIN control_material_almacen_prod cma ON iai.warehousing_id = cma.id
      ...
      for (const item of missingItems) {
        ...
        await connection.query(`
          INSERT INTO control_material_salida_prod (
        ...
        await connection.query(`
          UPDATE inventory_audit_item_prod 
    ...
    await connection.query(`
      UPDATE inventory_audit_prod SET
        status = 'Completed',
        fecha_fin = NOW(),
        usuario_fin = ?,
        verified_locations = ?,
...
const getAuditLocations = async (req, res, next) => {
  try {
    ...
    if (!auditId) {
      ...
      SELECT id FROM inventory_audit_prod WHERE status = 'InProgress' LIMIT 1
    `);
    ...
    const [locations] = await pool.query(`
      SELECT 
        ial.*,
        (SELECT COUNT(*) FROM inventory_audit_item_prod iai 
      ...
      FROM inventory_audit_location_prod ial
      WHERE ial.audit_id = ?
...
const getLocationItems = async (req, res, next) => {
  try {
    ...
    // Si no existe registro en inventory_audit_item_prod, el status es Pending
    const [items] = await pool.query(`
      ...
      WHERE cma.ubicacion_salida = ?
        ...
        AND (cma.estado_desecho = 0 OR cma.estado_desecho IS NULL)
        AND (cma.tiene_salida = 0 OR cma.tiene_salida IS NULL)
      ORDER BY cma.codigo_material_recibido
    `, [auditId, location]);
...
const scanLocation = async (req, res, next) => {
  try {
    ...
    // Verificar que la ubicacion exista en inventory_audit_location_prod
    const [loc] = await pool.query(`
      SELECT * FROM inventory_audit_location_prod 
    ...
    if (loc[0].status === 'Pending') {
      await pool.query(`
        UPDATE inventory_audit_location_prod 
...
const scanItem = async (req, res, next) => {
  try {
    ...
    // Buscar el material por codigo de etiqueta (codigo_material_recibido)
    const [material] = await pool.query(`
      SELECT id, codigo_material_recibido, numero_parte, ubicacion_salida, cantidad_actual
      FROM control_material_almacen_prod
    ...
    if (location && mat.ubicacion_salida !== location) {
      return res.json({
        success: false,
    ...
    const [existing] = await pool.query(`
      SELECT id, status FROM inventory_audit_item_prod
    ...
    // Registrar escaneo: crea o actualiza inventory_audit_item_prod como Found
    if (existing.length > 0) {
      await pool.query(`
        UPDATE inventory_audit_item_prod SET
          status = 'Found',
          scanned_at = NOW(),
          scanned_by = ?
        WHERE id = ?
    ...
    // Verificar si la ubicacion esta completa
    // Si total esperado == escaneados, marca la ubicacion Verified
    // Nota: el total se calcula desde control_material_almacen_prod (inventario activo)
    const [locationStats] = await pool.query(`
      SELECT 
        (SELECT COUNT(*) FROM control_material_almacen_prod 
         WHERE ubicacion_salida = ? AND cantidad_actual > 0 
         AND (cancelado = 0 OR cancelado IS NULL)
         AND (estado_desecho = 0 OR estado_desecho IS NULL)
         AND (tiene_salida = 0 OR tiene_salida IS NULL)) as total,
        (SELECT COUNT(*) FROM inventory_audit_item_prod 
         WHERE audit_id = ? AND location = ? AND status = 'Found') as scanned
    `, [mat.ubicacion_salida, auditId, mat.ubicacion_salida]);

    const isLocationComplete = locationStats[0].total === locationStats[0].scanned;

    if (isLocationComplete) {
      await pool.query(`
        UPDATE inventory_audit_location_prod SET
          status = 'Verified',
          completed_at = NOW()
        WHERE audit_id = ? AND location = ?
...
// Crea o actualiza inventory_audit_item_prod con status Missing y pone la ubicacion en Discrepancy.
const markMissing = async (req, res, next) => {
  try {
    ...
    const [material] = await pool.query(`
      SELECT id, codigo_material_recibido, ubicacion_salida
      FROM control_material_almacen_prod WHERE id = ?
    `, [warehousing_id]);
    ...
    // Verificar si ya existe registro
    const [existing] = await pool.query(`
      SELECT id FROM inventory_audit_item_prod
      WHERE audit_id = ? AND warehousing_id = ?
...
const completeLocation = async (req, res, next) => {
  try {
    ...
    const [pending] = await pool.query(`
      SELECT cma.id, cma.codigo_material_recibido
      FROM control_material_almacen_prod cma
      LEFT JOIN inventory_audit_item_prod iai ON iai.warehousing_id = cma.id AND iai.audit_id = ?
      WHERE cma.ubicacion_salida = ?
        AND cma.cantidad_actual > 0
...
const getAuditHistory = async (req, res, next) => {
  try {
    ...
    let query = `
      SELECT 
        ia.*,
        (SELECT COUNT(*) FROM inventory_audit_location_prod ial WHERE ial.audit_id = ia.id) as total_locations_count
      FROM inventory_audit_prod ia
...
const getAuditHistoryDetail = async (req, res, next) => {
  try {
    ...
    const [audit] = await pool.query(`
      SELECT * FROM inventory_audit_prod WHERE id = ?
    `, [id]);
    ...
    // Ubicaciones
    const [locations] = await pool.query(`
      SELECT * FROM inventory_audit_location_prod WHERE audit_id = ? ORDER BY location
    `, [id]);
...
const getAuditSummary = async (req, res, next) => {
  try {
    // Buscar auditoria activa (solo InProgress)
    const [active] = await pool.query(`
      SELECT * FROM inventory_audit_prod WHERE status = 'InProgress' LIMIT 1
    `);
...
const compareAudits = async (req, res, next) => {
  try {
    ...
    // Hacer JOIN con control_material_almacen_prod para obtener numero_parte y cantidad
...
const getLocationSummary = async (req, res, next) => {
  try {
    ...
    // Obtener resumen por parte de inventory_audit_part_prod
    const [parts] = await pool.query(`
      SELECT 
        ...
        iap.flagged_by,
        iap.flagged_at
      FROM inventory_audit_part_prod iap
      WHERE iap.audit_id = ? AND iap.location = ?
      ORDER BY iap.numero_parte
    `, [auditId, normalizedLocation]);

    // Si no hay registros en inventory_audit_part_prod (auditorias antiguas), generarlos al vuelo
    if (parts.length === 0) {
...
const confirmPart = async (req, res, next) => {
  try {
    ...
    const [result] = await pool.query(`
      UPDATE inventory_audit_part_prod 
    ...
    // Crear registros Found en inventory_audit_item_prod para todas las etiquetas de esta parte
    const [items] = await pool.query(`
      SELECT id, codigo_material_recibido, ubicacion_salida
      FROM control_material_almacen_prod
      WHERE ubicacion_salida = ? AND numero_parte = ?
        AND cantidad_actual > 0
        AND (cancelado = 0 OR cancelado IS NULL)
        AND (estado_desecho = 0 OR estado_desecho IS NULL)
        AND (tiene_salida = 0 OR tiene_salida IS NULL)
    ...
    for (const item of items) {
      ...
      if (existing.length === 0) {
        ...
        await pool.query(`
          UPDATE inventory_audit_item_prod SET status = 'Found', scanned_at = NOW(), scanned_by = ?
          WHERE id = ?
        `, [usuario || 'Mobile', existing[0].id]);
      }
    }
...
const flagMismatch = async (req, res, next) => {
  try {
    ...
    await pool.query(`
      UPDATE inventory_audit_item_prod iai
      JOIN control_material_almacen_prod cma ON iai.warehousing_id = cma.id
...
const scanPartItem = async (req, res, next) => {
  try {
    ...
    const [partRecord] = await pool.query(`
      SELECT id, status FROM inventory_audit_part_prod
    ...
    const [updatedPart] = await pool.query(`
      SELECT expected_items, scanned_items FROM inventory_audit_part_prod WHERE id = ?
    `, [partRecord[0].id]);
...
const confirmMissing = async (req, res, next) => {
  ...
  try {
    ...
    const [partRecord] = await connection.query(`
      ...
      FROM inventory_audit_part_prod
      WHERE audit_id = ? AND location = ? AND numero_parte = ?
    ...
    const [unscanned] = await connection.query(`
      SELECT cma.id, cma.codigo_material_recibido, cma.numero_lote_material, cma.cantidad_actual, cma.especificacion
      FROM control_material_almacen_prod cma
      LEFT JOIN inventory_audit_item_prod iai ON iai.warehousing_id = cma.id AND iai.audit_id = ?
      WHERE cma.ubicacion_salida = ? AND cma.numero_parte = ?
        AND cma.cantidad_actual > 0
        AND (cma.cancelado = 0 OR cma.cancelado IS NULL)
        AND (cma.estado_desecho = 0 OR cma.estado_desecho IS NULL)
        AND (cma.tiene_salida = 0 OR cma.tiene_salida IS NULL)
    ...
    for (const item of unscanned) {
      ...
      // 2. Crear salida en control_material_salida_prod
      await connection.query(`
        INSERT INTO control_material_salida_prod (
...
async function checkLocationCompletion(auditId, location) {
  const [parts] = await pool.query(`
    SELECT status FROM inventory_audit_part_prod
    WHERE audit_id = ? AND location = ?
  `, [auditId, location]);
...
const getPendingApprovals = async (req, res, next) => {
  try {
    ...
    const [items] = await pool.query(`
      ...
      FROM inventory_audit_item_prod iai
      JOIN control_material_almacen_prod cma ON iai.warehousing_id = cma.id
      JOIN inventory_audit_prod ia ON iai.audit_id = ia.id
      WHERE ${whereClause}
      ORDER BY iai.location, cma.numero_parte
    `, params);
...
const approveDiscrepancy = async (req, res, next) => {
  ...
  try {
    ...
    const [pendingItems] = await connection.query(`
      SELECT iai.*, cma.numero_lote_material, cma.cantidad_actual, cma.especificacion
      FROM inventory_audit_item_prod iai
      JOIN control_material_almacen_prod cma ON iai.warehousing_id = cma.id
      WHERE iai.audit_id = ? 
        AND iai.location = ? 
    ...
    const [remainingPending] = await connection.query(`
      SELECT COUNT(*) as cnt FROM inventory_audit_part_prod
