...
exports.requestCancellation = async (req, res, next) => {
  try {
    ...
    const [entry] = await pool.query(
      'SELECT id, codigo_material_recibido, cancelado FROM control_material_almacen_prod WHERE id = ?',
      [warehousingId]
    );
...
exports.getPendingRequests = async (req, res, next) => {
  try {
    const [requests] = await pool.query(`
      ...
      LEFT JOIN control_material_almacen_prod cma ON cr.warehousing_id = cma.id
      WHERE cr.status = ?
...
exports.getStatus = async (req, res, next) => {
  try {
    ...
    const [entry] = await pool.query(
      'SELECT cancelado FROM control_material_almacen_prod WHERE id = ?',
      [warehousingId]
    );
