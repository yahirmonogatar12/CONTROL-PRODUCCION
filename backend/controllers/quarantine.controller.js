...
exports.getAll = async (req, res, next) => {
  try {
    ...
    let query = `SELECT * FROM quarantine_prod`;
    const params = [];
...
exports.getHistory = async (req, res, next) => {
  try {
    ...
    const [rows] = await pool.query(
      `SELECT * FROM quarantine_history_prod WHERE quarantine_id = ? ORDER BY action_at DESC`,
      [id]
    );
    res.json(rows);
  } catch (err) {
...
exports.getAllHistory = async (req, res, next) => {
  try {
    ...
    let query = `SELECT q.*, 
                   (SELECT COUNT(*) FROM quarantine_history_prod WHERE quarantine_id = q.id) as history_count
                 FROM quarantine_prod q 
    ...
    for (const item of items) {
      try {
        const [existing] = await pool.query(
          `SELECT id, en_cuarentena FROM control_material_almacen_prod WHERE codigo_material_recibido = ?`,
        ...
        await pool.query(
          `INSERT INTO quarantine_history_prod (quarantine_id, action, new_status, comments, action_by_id, action_by_name)
           VALUES (?, 'Created', 'InQuarantine', ?, ?, ?)`,
          [quarantineId, reason, userId, userName]
        );
        
        await pool.query(
          `UPDATE control_material_almacen_prod SET en_cuarentena = 1 WHERE codigo_material_recibido = ?`,
          [item.codigo_material_recibido]
        );
...
exports.update = async (req, res, next) => {
  try {
    ...
    const [current] = await pool.query(
      `SELECT status, codigo_material_recibido, cantidad FROM quarantine_prod WHERE id = ?`, 
      [id]
    );
    if (current.length === 0) {
    ...
    await pool.query(`UPDATE quarantine_prod SET ${updateFields.join(', ')} WHERE id = ?`, updateValues);
    
    await pool.query(
      `INSERT INTO quarantine_history_prod (quarantine_id, action, old_status, new_status, comments, action_by_id, action_by_name)
       VALUES (?, ?, ?, ?, ?, ?, ?)`,
    ...
    if (status === 'Released') {
      await pool.query(
        `UPDATE control_material_almacen_prod SET en_cuarentena = 0 WHERE codigo_material_recibido = ?`,
        [codigo]
      );
    }
    ...
    if (status === 'Scrapped' || status === 'Returned') {
      await pool.query(
        `UPDATE control_material_almacen_prod SET en_cuarentena = 0, estado_desecho = 1 WHERE codigo_material_recibido = ?`,
        [codigo]
      );
      
      await pool.query(
        `UPDATE inventario_lotes_prod SET total_salida = total_entrada WHERE codigo_material_recibido = ?`,
        [codigo]
      );
    }
...
exports.addComment = async (req, res, next) => {
  try {
    ...
    const [current] = await pool.query(`SELECT status FROM quarantine_prod WHERE id = ?`, [id]);
    if (current.length === 0) {
      return res.status(404).json({ error: 'Quarantine item not found' });
    }
    ...
    await pool.query(`UPDATE quarantine_prod SET updated_at = NOW() WHERE id = ?`, [id]);
