...
exports.getAll = async (req, res, next) => {
  try {
    const [rows] = await pool.query(`
      ...
      FROM material_return_prod mr
      LEFT JOIN control_material_almacen_prod cma ON mr.warehousing_id = cma.id
...
exports.create = async (req, res, next) => {
  try {
    ...
    if (warehousing_id) {
      const [rows] = await pool.query(
        'SELECT cantidad_actual FROM control_material_almacen_prod WHERE id = ?',
        [warehousing_id]
      );
      if (rows.length > 0) {
    ...
    const [result] = await pool.query(`
      INSERT INTO material_return_prod (
        warehousing_id,
        material_warehousing_code,
        part_number,
        material_lot_no,
    ...
    if (warehousing_id) {
      // Obtener datos del registro original para actualizar inventario_lotes_prod
      const [originalData] = await pool.query(
        'SELECT numero_parte, numero_lote_material, codigo_material_recibido FROM control_material_almacen_prod WHERE id = ?',
        [warehousing_id]
      );
      ...
      // Actualizar inventario_lotes_prod - RESTAR la cantidad devuelta del total_salida
...
exports.delete = async (req, res, next) => {
  try {
    ...
    const [returns] = await pool.query('SELECT * FROM material_return_prod WHERE id = ?', [id]);
    ...
    await pool.query('DELETE FROM material_return_prod WHERE id = ?', [id]);
