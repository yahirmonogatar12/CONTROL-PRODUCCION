...
const getAll = async (req, res, next) => {
  try {
    const [rows] = await pool.query(`
      ...
      LEFT JOIN quarantine_prod q ON cma.id = q.warehousing_id
...
const create = async (req, res, next) => {
  try {
    ...
    const [result] = await pool.query(`
      INSERT INTO control_material_almacen_prod (
    ...
    // Si requiere IQC, crear registro en iqc_inspection_lot_prod
...
const update = async (req, res, next) => {
  try {
    ...
    const [result] = await pool.query(
      `UPDATE control_material_almacen_prod SET ${setClause} WHERE id = ?`,
      values
    );
...
const deleteEntry = async (req, res, next) => {
  try {
    const [result] = await pool.query(
      'UPDATE control_material_almacen_prod SET cancelado = 1 WHERE id = ?',
      [req.params.id]
    );
...
const searchWarehouseMaterial = async (req, res, next) => {
  try {
    ...
    // Verificar si ya fue confirmado en SMD (ya existe en control_material_almacen_prod)
...
const confirmFromWarehouseByIds = async (req, res, next) => {
  ...
  try {
    ...
    const [existentes] = await connection.query(`
      SELECT codigo_material_recibido FROM control_material_almacen_prod
