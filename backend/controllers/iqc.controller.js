...
exports.getLotByLabel = async (req, res, next) => {
  try {
    ...
    const [inspections] = await pool.query(
      'SELECT * FROM iqc_inspection_lot_prod WHERE receiving_lot_code = ?',
      [receiving_lot_code]
    );
...
exports.getPending = async (req, res, next) => {
  try {
    const [rows] = await pool.query(`
      SELECT 
        il.*,
        (SELECT COUNT(*) FROM control_material_almacen_prod WHERE receiving_lot_code = il.receiving_lot_code) as label_count
      FROM iqc_inspection_lot_prod il
...
exports.countPending = async (req, res, next) => {
  try {
    const [rows] = await pool.query(`
      SELECT COUNT(*) as count FROM iqc_inspection_lot_prod 
...
exports.getById = async (req, res, next) => {
  try {
    const [rows] = await pool.query(
      'SELECT * FROM iqc_inspection_lot_prod WHERE id = ?',
      [req.params.id]
    );
    if (rows.length === 0) {
    ...
    const [details] = await pool.query(
      'SELECT * FROM iqc_inspection_detail_prod WHERE inspection_lot_id = ? ORDER BY sample_number, characteristic',
      [req.params.id]
    );
...
exports.create = async (req, res, next) => {
  try {
    ...
    if (existing.length > 0) {
      ...
      await pool.query(`
        UPDATE iqc_inspection_lot_prod 
      ...
      const [maxSeq] = await pool.query(
        'SELECT MAX(COALESCE(lot_sequence, 1)) as max_seq FROM iqc_inspection_lot_prod WHERE receiving_lot_code = ?',
        [receiving_lot_code]
      );
      currentLotSequence = (maxSeq[0].max_seq || 0) + 1;
      
      const [result] = await pool.query(`
        INSERT INTO iqc_inspection_lot_prod (
...
exports.updateResult = async (req, res, next) => {
  try {
    ...
    const [updateResult] = await pool.query(
      `UPDATE iqc_inspection_lot_prod SET ${field} = ?, updated_at = NOW() WHERE id = ?`,
      [result, id]
    );
...
exports.update = async (req, res, next) => {
  try {
    ...
    const [result] = await pool.query(
      `UPDATE iqc_inspection_lot_prod SET ${setClause}, updated_at = NOW() WHERE id = ?`,
      values
    );
...
exports.close = async (req, res, next) => {
  try {
    ...
    const [inspections] = await pool.query(
      'SELECT receiving_lot_code, COALESCE(lot_sequence, 1) as lot_sequence FROM iqc_inspection_lot_prod WHERE id = ?',
      [id]
    );
...
exports.addDetail = async (req, res, next) => {
  try {
    ...
    const [insertResult] = await pool.query(`
      INSERT INTO iqc_inspection_detail_prod (
...
exports.deleteDetail = async (req, res, next) => {
  try {
    const [result] = await pool.query(
      'DELETE FROM iqc_inspection_detail_prod WHERE id = ?',
      [req.params.id]
    );
...
exports.saveMeasurements = async (req, res, next) => {
  try {
    ...
    await pool.query('DELETE FROM iqc_inspection_detail_prod WHERE inspection_lot_id = ?', [inspectionId]);
...
exports.getMeasurements = async (req, res, next) => {
  try {
    ...
    const [rows] = await pool.query(`
      ...
      FROM iqc_inspection_detail_prod 
