...
const validateBatch = async (req, res, next) => {
  try {
    ...
    const [rows] = await pool.query(`
      ...
      FROM control_material_almacen_prod c
      LEFT JOIN materiales m ON c.numero_parte = m.numero_parte
      WHERE UPPER(c.codigo_material_recibido) = UPPER(?)
      LIMIT 1
    `, [code]);
    ...
    if (material.numero_lote_material) {
      ...
      if (blacklistCheck.length > 0) {
        const reason = blacklistCheck[0].reason || 'Sin razón especificada';
        return res.json({ 
          valid: false, 
          error: `El lote ${material.numero_lote_material} está en lista negra. Razón: ${reason}`,
...
const getAll = async (req, res, next) => {
  try {
    const [rows] = await pool.query(`
      ...
      LEFT JOIN control_material_almacen_prod cma ON cms.codigo_material_recibido = cma.codigo_material_recibido
...
const splitLot = async (req, res, next) => {
  ...
  try {
    ...
    // 4. Crear registros en control_material_almacen_prod para cada pack dividido
    const newLabels = [];
    const outgoingIds = [];
