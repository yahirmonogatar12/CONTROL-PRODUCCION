...
// Cache de tablas que ya sabemos que no existen (evita logs repetitivos)
const _missingTablesCache = new Set();

// Helper para verificar si una tabla existe
async function tableExists(table) {
  if (_missingTablesCache.has(table)) return false;
  try {
...
async function addCanceladoColumn() {
  const tables = [
    'control_material_almacen_prod',
    'control_material_salida',
    'control_material_salida_prod'
  ];
...
async function addTieneSalidaColumn() {
  try {
    await pool.query(`
      ALTER TABLE control_material_almacen_prod 
...
// Agregar columnas IQC a control_material_almacen_prod
async function addIqcColumns() {
  ...
  for (const col of columns) {
    await addColumnIfNotExists('control_material_almacen_prod', col.name, col.definition);
  }
...
async function createIqcTables() {
  try {
    // Tabla iqc_inspection_lot_prod
    await pool.query(`
      CREATE TABLE IF NOT EXISTS iqc_inspection_lot_prod (
    ...
    try {
      await pool.query(`ALTER TABLE iqc_inspection_lot_prod MODIFY COLUMN material_code TEXT NULL`);
      await pool.query(`ALTER TABLE iqc_inspection_lot_prod MODIFY COLUMN part_number VARCHAR(150) NULL`);
      await pool.query(`ALTER TABLE iqc_inspection_lot_prod MODIFY COLUMN receiving_lot_code VARCHAR(50) NOT NULL`);
    } catch (e) { }
    ...
    for (const col of iqcLotColumns) {
      await addColumnIfNotExists('iqc_inspection_lot_prod', col.name, col.definition);
    }
    ...
    for (const col of resultColumns) {
      try {
        await pool.query(`ALTER TABLE iqc_inspection_lot_prod MODIFY COLUMN ${col} VARCHAR(20) DEFAULT 'Pending'`);
      } catch (e) { }
    }

    // Manejar índices
    try {
      await pool.query(`ALTER TABLE iqc_inspection_lot_prod DROP INDEX receiving_lot_code`);
    } catch (e) { }

    try {
      await pool.query(`
        CREATE UNIQUE INDEX idx_receiving_lot_sequence 
        ON iqc_inspection_lot_prod(receiving_lot_code, lot_sequence)
      `);
    } catch (e) { }

    // Tabla iqc_inspection_detail_prod
    await pool.query(`
      CREATE TABLE IF NOT EXISTS iqc_inspection_detail_prod (
        id INT AUTO_INCREMENT PRIMARY KEY,
...
async function addUsuarioRegistroColumns() {
  const tables = [
    { table: 'control_material_almacen_prod', name: 'usuario_registro', definition: 'VARCHAR(150) NULL' },
    { table: 'control_material_salida_prod', name: 'usuario_registro', definition: 'VARCHAR(150) NULL' },
    { table: 'control_material_salida', name: 'usuario_registro', definition: 'VARCHAR(150) NULL' },
    { table: 'control_material_entrada_prod', name: 'usuario_registro', definition: 'VARCHAR(150) NULL' }
  ];
...
async function createControlMaterialEntradaSmdTable() {
  try {
    await pool.query(`
      CREATE TABLE IF NOT EXISTS control_material_entrada_prod (
        id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    await addColumnIfNotExists('control_material_entrada_prod', 'confirmado', 'TINYINT DEFAULT 0');
    await addColumnIfNotExists('control_material_entrada_prod', 'confirmado_por', 'VARCHAR(150) NULL');
    await addColumnIfNotExists('control_material_entrada_prod', 'confirmado_at', 'DATETIME NULL');
    await addColumnIfNotExists('control_material_entrada_prod', 'rechazado', 'TINYINT DEFAULT 0');
    await addColumnIfNotExists('control_material_entrada_prod', 'rechazado_por', 'VARCHAR(150) NULL');
    await addColumnIfNotExists('control_material_entrada_prod', 'rechazado_at', 'DATETIME NULL');
    await addColumnIfNotExists('control_material_entrada_prod', 'rechazado_motivo', 'TEXT NULL');

    console.log('? Tabla control_material_entrada_prod verificada/creada');
  } catch (err) {
    console.log('Nota: La tabla control_material_entrada_prod puede ya existir:', err.message);
  }
}
...
async function createQuarantineTables() {
  try {
    await pool.query(`
      CREATE TABLE IF NOT EXISTS quarantine_prod (
        id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS quarantine_history_prod (
        id INT AUTO_INCREMENT PRIMARY KEY,
...
async function createAuditTables() {
  try {
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS inventory_audit_prod (
        id INT AUTO_INCREMENT PRIMARY KEY,
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS inventory_audit_location_prod (
        id INT AUTO_INCREMENT PRIMARY KEY,
        ...
        FOREIGN KEY (audit_id) REFERENCES inventory_audit_prod(id) ON DELETE CASCADE,
    ...
    await pool.query(`
      CREATE TABLE IF NOT EXISTS inventory_audit_item_prod (
        id INT AUTO_INCREMENT PRIMARY KEY,
        audit_id INT NOT NULL,
        warehousing_id INT NOT NULL,
...
async function createAuditPartTable() {
  try {
    await pool.query(`
      CREATE TABLE IF NOT EXISTS inventory_audit_part_prod (
        id INT AUTO_INCREMENT PRIMARY KEY,
        audit_id INT NOT NULL,
        location VARCHAR(100) NOT NULL,
    ...
    console.log('✓ Tabla inventory_audit_part_prod verificada/creada');
  } catch (err) {
    console.log('Nota: La tabla inventory_audit_part_prod puede ya existir:', err.message);
  }
}

// ============================================
// TABLA PCB INVENTORY SCAN PROD
// ============================================
async function createPcbInventoryScanTable() {
  try {
    await pool.query(`
      CREATE TABLE IF NOT EXISTS pcb_inventory_scan_prod (
    ...
    console.log('✓ Tabla pcb_inventory_scan_prod verificada/creada');
  } catch (err) {
    console.log('Nota: La tabla pcb_inventory_scan_prod puede ya existir:', err.message);
  }
}
...
async function migratePcbInventorySchema() {
  ...
  try {
    const [procesoCols] = await pool.query(`
      ...
      WHERE TABLE_SCHEMA = DATABASE()
      AND TABLE_NAME = 'pcb_inventory_scan_prod'
    ...
    if (procesoNeedsMigration) {
      await pool.query(`
        ALTER TABLE pcb_inventory_scan_prod
  ...
  try {
    await pool.query(`
      ...
      ON pcb_inventory_scan_prod (area)
    `);
  ...
  try {
    await pool.query(`
      ...
      ON pcb_inventory_scan_prod (array_group_code)
    `);
...
// Agregar columna tipo_movimiento a pcb_inventory_scan_prod si no existe
async function addPcbInventoryTipoMovimiento() {
  ...
  try {
    await pool.query(`
      ...
      ON pcb_inventory_scan_prod (inventory_date, scanned_original_norm, tipo_movimiento, area)
    `);
  ...
  if (hasAreaUniqueIndex) {
    await dropIndexIfExists('pcb_inventory_scan_prod', 'uk_daily_original');
    await dropIndexIfExists('pcb_inventory_scan_prod', 'uk_daily_original_tipo');
  }

  try {
    await pool.query(`
      CREATE INDEX idx_daily_tipo 
      ON pcb_inventory_scan_prod (inventory_date, tipo_movimiento)
    `);
...
async function runMigrations() {
  ...
  if (_missingTablesCache.size > 0) {
    console.log(`Nota: Tablas no encontradas (se omitieron migraciones de columnas): ${[..._missingTablesCache].join(', ')}`);
  }
...
async function addReentryColumns() {
  ...
  try {
    await pool.query(`CREATE INDEX idx_fecha_reingreso ON control_material_almacen_prod(fecha_reingreso)`);
  } catch (err) {
