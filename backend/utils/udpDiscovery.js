...
function startDiscoveryService(httpPort) {
  ...
  server.on('error', (err) => {
    if (err.code === 'EADDRINUSE' || err.code === 'EACCES') {
      console.log(`Nota: Puerto UDP Discovery no disponible (${err.code}), reintentando en 5s...`);
      try { server.close(); } catch (_e) { /* ya cerrado */ }
      setTimeout(() => {
        try {
          const retryServer = dgram.createSocket({ type: 'udp4', reuseAddr: true });
          retryServer.on('error', (retryErr) => {
            console.log('Nota: UDP Discovery no disponible:', retryErr.message);
            try { retryServer.close(); } catch (_e) { /* ya cerrado */ }
          });
          retryServer.on('listening', () => {
            isBound = true;
            retryServer.setBroadcast(true);
            beaconInterval = setInterval(() => {
              ...
              retryServer.send(beacon, DISCOVERY_PORT, broadcastAddr, () => {});
            }, BEACON_INTERVAL);
          });
          retryServer.bind(DISCOVERY_PORT);
        } catch (_e) {
          // Silenciar - el servicio UDP no es critico
        }
      }, 5000);
      return;
    }
